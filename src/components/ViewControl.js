...
function ViewControl() {
  ...
  const handleAddingNewArtistToList = async (newArtistData) => {
  ...
  if (auth.currentUser == null) {
    ...
    if (error) {
      ...
      currentlyVisibleState = (
        <NewArtistForm onNewArtistCreation={handleAddingNewArtistToList} />
      );
    ...
    return (
      <React.Fragment>
        <div className="artist-container">
          {currentlyVisibleState}
          {!error && (
            <button className="add-artist-button" onClick={handleClick}>
              {buttonText}
            </button>
          )}
        </div>
